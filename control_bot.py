...
def get_category_toggle_button(
    category: str,
    settings: SettingsService,
) -> str:
    category_targets = TARGET_MONSTER_CATEGORIES.get(category, [])
    selected_targets = set(settings.values.enabled_targets or [])
    category_enabled = bool(category_targets) and all(
        target in selected_targets for target in category_targets
    )
    prefix = (
        TARGETS_DISABLE_ALL_PREFIX
        if category_enabled
        else TARGETS_ENABLE_ALL_PREFIX
    )
...
def get_targets_text(
    ...
    lines = [
        f"🎯 {category}",
        "",
        f"{icon} Выбрано мобов: {enabled_count}/{total_count}",
        "",
    ]
    lines.extend(
        f"{'✅' if target in selected_targets else '❌'} {target}"
...
class ControlBot:
    def __init__(
        ...
        self.router.message.outer_middleware(
            AdminOnlyMiddleware(ADMIN_TELEGRAM_ID)
        )
    ...
    async def _send_text(
        ...
        try:
            reply_markup = (
                keyboard(rows)
                if rows is not None
                else await self.current_keyboard()
            )
            ...
            logger.warning(
                "Не удалось отправить ответ панели из-за сетевой ошибки"
            )
    ...
    def _register_handlers(self) -> None:
        ...
        async def cycles_input(
            ...
            await self._send_text(
                message,
                f"✅ Количество циклов: {value}",
                SETTINGS_ROWS,
            )

        @r.message(StateFilter(None), F.text == "👣 Ходов в цикле")
        async def moves_prompt(
            message: Message,
            state: FSMContext,
        ) -> None:
        ...
        async def character_value_prompt(message: Message, state: FSMContext) -> None:
            ...
            await self._send_text(
                message,
                f"Отправьте {label} целым числом.",
                [["❌ Отмена"]],
            )
        ...
        async def character_value_input(message: Message, state: FSMContext) -> None:
            ...
            await self.settings.set_value(key, value)
            await state.clear()
            v = self.settings.values
            await self._send_text(
                message,
        ...
        @r.message(
            StateFilter(None),
            F.text.startswith(TARGETS_ENABLE_ALL_PREFIX),
        )
        async def enable_target_category(message: Message) -> None:
            ...
            await self._send_text(
                message,
                f"✅ Все мобы локации «{category}» выбраны.\n\n"
                f"{get_targets_text(category, self.settings)}",
                get_targets_keyboard(category, self.settings),
            )

        @r.message(
            StateFilter(None),
            F.text.startswith(TARGETS_DISABLE_ALL_PREFIX),
        )
        async def disable_target_category(message: Message) -> None:
            ...
            await self._send_text(
                message,
                f"❌ Все мобы локации «{category}» отключены.\n\n"
                f"{get_targets_text(category, self.settings)}",
                get_targets_keyboard(category, self.settings),
            )
        ...
        @r.message(
            StateFilter(None),
            F.text.in_(DEFAULT_TARGET_MONSTERS),
        )
        ...
        @r.message(
            StateFilter(None),
            F.text.in_(set(delay_mapping)),
        )
        async def delay_prompt(
            message: Message,
            state: FSMContext,
        ) -> None:
        ...
        async def delay_input(
            ...
            await self._send_text(
                message,
                "✅ Диапазон задержки сохранён.",
                DELAYS_ROWS,
            )

        @r.message(
            StateFilter(None),
            F.text == "🎲 Шанс длинной паузы",
        )
        async def chance_prompt(
            message: Message,
            state: FSMContext,
        ) -> None:
    ...
    async def start(self) -> None:
        ...
        self.polling_task = asyncio.create_task(
            self.dispatcher.start_polling(
                self.bot,
                allowed_updates=(
                    self.dispatcher.resolve_used_update_types()
                ),
