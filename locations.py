...
def _build_location(name: str, monsters: list[str]) -> Location:
    geometry = _LOCATION_GEOMETRY.get(name, _DEFAULT_GEOMETRY)
    return Location(
        key=name.casefold().replace(" ", "_"),
        name=name,
        monsters=tuple(monsters),
        aggressive_monsters=frozenset(
            monster for monster in monsters if monster in AGGRESSIVE_MONSTERS
        ),
...
LOCATIONS = {
    name: _build_location(name, monsters) for name, monsters in TARGET_MONSTER_CATEGORIES.items()
}
...
def location_for_monster(monster: str) -> Location | None:
    return next(
        (location for location in LOCATIONS.values() if monster in location.monsters),
        None,
    )
