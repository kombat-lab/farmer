...
def print_buttons(message) -> None:
    ...
    for row_index, row in enumerate(message.buttons):
        labels = [
            f"[{row_index},{column_index}] {getattr(button, 'text', '')!r}"
...
def print_parsed_state(text: str) -> None:
    ...
    print(f"  Список сокращён: {'ДА' if map_info.has_hidden_monsters else 'НЕТ'}")
    print(f"  Найденная цель: {map_info.found_target or 'нет'}")
    print(f"  Переход завершён: {'ДА' if map_info.movement_finished else 'НЕТ'}")
...
def print_message(message, event_type: str) -> None:
    ...
    print(f"Локальное время: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Дата сообщения: {format_date(message.date)}")
    print(f"ID сообщения: {message.id}")
    print(f"Исходящее: {message.out}")
    print(f"Отредактировано: {'да' if message.edit_date else 'нет'}")

    if message.edit_date:
        print(f"Дата редактирования: {format_date(message.edit_date)}")
...
async def main() -> None:
    ...
    try:
        ...
        print(f"\nОшибка Telegram API: {type(error).__name__}: {error}")
    except Exception as error:
        print(f"\nНеожиданная ошибка: {type(error).__name__}: {error}")
    finally:
