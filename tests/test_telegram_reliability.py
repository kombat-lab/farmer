...
class EventFlowTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        with patch('farmer.TelegramClient', return_value=self.client):
            self.farmer = Farmer(self.storage, notifier, settings)
        self.farmer.game_bot = 'offline-test-peer'
        self.farmer.context.current_hp = 780
        self.farmer.context.max_hp = 780
    ...
    async def drain(self, *messages: Message) -> None:
        ...
        self.farmer.worker_task = asyncio.create_task(self.farmer.event_worker())
        await asyncio.wait_for(self.farmer.event_queue.join(), timeout=2)
    ...
    async def test_victory_is_recorded_before_following_map(self) -> None:
        ...
        self.assertEqual(self.farmer.statistics.session_wins, 1)
    ...
    async def test_defeat_starts_recovery_even_with_a_newer_map(self) -> None:
        ...
        self.assertEqual(self.farmer.statistics.session_defeats, 1)
        self.assertIs(self.farmer.state, BotState.RECOVERY)
        self.assertIsNotNone(self.farmer.recovery_task)
    ...
    async def test_reprocessing_prompt_does_not_learn_round_twice(self) -> None:
        ...
        first_samples = self.farmer.combat.incoming_damage.samples
        await self.farmer.handle_message(message)
        self.assertEqual(first_samples, 1)
        self.assertEqual(self.farmer.combat.incoming_damage.samples, first_samples)
    ...
    async def test_stale_combat_prompt_cannot_start_a_skill_action(self) -> None:
        ...
        self.farmer.click_button.assert_not_awaited()

    async def test_full_queue_backpressures_without_dropping_battle_result(self) -> None:
        self.farmer.event_queue = asyncio.Queue(maxsize=1)
    ...
    async def test_refresh_retries_after_no_response_without_replaying_buttons(self) -> None:
        ...
        self.assertTrue(await self.farmer.request_map_refresh())
        self.assertFalse(await self.farmer.request_map_refresh())
        now = 31
        self.assertTrue(await self.farmer.request_map_refresh())
    ...
    async def test_hanging_refresh_times_out_and_releases_gate(self) -> None:
        ...
        with patch('farmer.TELEGRAM_MAP_RPC_TIMEOUT', 0.01):
            self.assertFalse(await self.farmer.request_map_refresh())
        now = 31
        self.client.send_message.side_effect = None
        self.assertTrue(await self.farmer.request_map_refresh())
    ...
    async def test_recovery_stops_after_bounded_unsuccessful_attempts(self) -> None:
        self.farmer.watchdog.recovery_attempts = 3
    ...
    async def test_keyboard_edit_updates_ui_without_learning_damage_twice(self) -> None:
        ...
        self.assertEqual(self.farmer.combat.incoming_damage.samples, 1)
        latest = self.farmer.combat.latest_round
    ...
    async def test_cooldown_started_while_waiting_prevents_callback(self) -> None:
        ...
        self.farmer.latest_received_message = message
        self.farmer.latest_messages[message.id] = message
        ...
        self.assertNotIn(
            (0, message_fact_key(message)),
            self.farmer.attempted_actions,
        )
    ...
    async def test_keyboard_only_map_edit_does_not_reject_pending_move(self) -> None:
        ...
        await self.farmer.enqueue_message(first)
        await self.farmer.handle_message(first)
        plan = self.farmer.navigator.plan((8, 0))
        self.farmer.context.pending_move = plan
        edited = Message(1, text, ((Button("b"),),))
        await self.farmer.enqueue_message(edited)
        await self.farmer.handle_message(edited)
        self.assertEqual(self.farmer.context.failed_move_attempts, 0)
        self.assertIs(self.farmer.context.pending_move, plan)
    ...
    async def test_map_roundtrip_observes_return_even_with_identical_keyboard(self) -> None:
        ...
        self.assertEqual(self.farmer.inbound_generation, 3)
        self.assertEqual(self.farmer.context.current_position, (8, 0))
    ...
    async def test_map_roundtrip_confirms_return_move_with_changed_keyboard(self) -> None:
        ...
        self.farmer.context.pending_move = MovePlan(
            (7, 0), (8, 0), "➡️", RouteDirection.DOWN, RouteDirection.DOWN
        )
        await self.drain(Message(1, "Позиция: (8, 0)\nМонстры на клетке: 0", ((Button("b"),),)))
        self.assertEqual(self.farmer.context.current_position, (8, 0))
        self.assertIsNone(self.farmer.context.pending_move)
        self.assertEqual(self.farmer.context.move_count, 1)
        self.assertEqual(self.farmer.context.failed_move_attempts, 0)
    ...
    async def test_countdown_edit_preserves_prompt_and_cannot_repeat_action(self) -> None:
        ...
        await self.farmer.enqueue_message(edited)
        self.assertEqual(self.farmer.inbound_generation, 1)
        self.assertEqual(self.farmer.event_queue.qsize(), 1)
    ...
    async def test_duplicate_old_map_delivery_does_not_reactivate_it(self) -> None:
        ...
        self.assertEqual(self.farmer.context.current_position, (7, 0))
        self.assertIs(self.farmer.latest_received_message, middle)
    ...
    async def test_returned_map_restores_location_and_geometry_at_same_position(self) -> None:
        ...
        for middle in transitions:
            with self.subTest(middle=middle):
                ...
                self.assertEqual(self.farmer.navigator.location_name, "Темный грот")
                self.assertEqual(self.farmer.navigator.max_x, 2)
                self.assertEqual(self.farmer.navigator.max_y, 2)
                self.assertEqual(self.farmer.context.current_position, (1, 0))
